...
def AE_gen(x,a,b,c):
    ...
    #kz = 0.98
    lamda = 0.15406
    FWHM=(T2-T1)*math.pi/180
    print("FWHM=",FWHM)
    costheta=np.cos(Tm/2)
    tantheta=np.tan(Tm/2)
...
for i in range(n_value):
    ledger.append(
        'P' + str(i+1)
                + ', D' + str(i+1) +' = '+ str(round(AE3[i],3))+ ' nm'
...
ax1.text(0.80, 0.85, r'DA = {0:0.6f} nm'
